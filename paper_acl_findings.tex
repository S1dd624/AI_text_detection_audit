...
\begin{abstract}
We present a forensic audit of AI text detectors using topic-controlled paired evaluation. Our Human Revision Study (HRS) comprises five domain-specific document pairs where each pair shares the same subject but differs in authorship. We audit a fine-tuned DeBERTaV3 classifier (AUROC $0.971$) and an SBERT baseline (AUROC $0.815$) through six complementary lenses: detector probability delta, $H_1$ topological density, Hypothesis Revision Index, cross-document BERTScore, and intra-document similarity. DeBERTa achieves near-maximal discrimination on three of four ground-truth pairs ($\Delta \in [-0.982, -0.906]$) but fails completely ($\Delta \approx 0$) on the pair whose topic is AI text detection---a \emph{meta-recursive domain confound}. A synthetic control pair (AI vs.~AI) correctly yields $\Delta \approx 0$. Topological features do not reach significance ($p = 0.433$, $N = 4$). We contribute an experimental design and audit framework revealing failure modes invisible to standard benchmarks.
\end{abstract}

\section{Introduction}

The evaluation of AI text detectors typically compares human and AI texts from \emph{different} topics, conflating topical signals with authorship~\cite{wang2024m4}. We introduce three contributions: (1)~a topic-controlled evaluation protocol (HRS) with five same-topic document pairs; (2)~a multi-lens forensic audit combining six complementary metrics; (3)~the first documented meta-recursive detection failure, where a detector fails because the text's topic overlaps with the detection task itself.
...
\paragraph{HRS Pairs.} Five domain pairs (Table~\ref{tab:pairs}): four ground-truth (human vs.~AI on the same topic) and one synthetic control (AI vs.~AI).
...
\begin{tabular}{@{}llcc@{}}
\toprule
\textbf{Domain} & \textbf{Type} & \textbf{AI} & \textbf{Hum} \\
\midrule
...
Three findings (Table~\ref{tab:hrs}):

\paragraph{(1) DeBERTa discriminates 3/4 pairs.} Education ($\Delta = -0.906$), policy ($\Delta = -0.978$), and sustainability ($\Delta = -0.982$) show near-maximal separation.

\paragraph{(2) Meta-recursive anomaly.} The detection pair ($\Delta \approx 0$) fails precisely because both texts discuss AI text detection. The subject matter overlaps with the features the detector learned, creating a domain confound where both texts score nearly identically on $P_{\text{AI}}$.

\paragraph{(3) Synthetic control validates design.} The AI-vs.-AI pair yields $\Delta = -0.004$.
...
\centering
\small
\caption{Permutation tests ($N = 4$ GT pairs).}
\label{tab:perm}
\begin{tabular}{@{}lccc@{}}
\toprule
...
The meta-recursive anomaly is our central finding. When a text's subject overlaps with the features a detector uses, the detector cannot separate authorship from content. This has practical implications: neural detectors may systematically fail on texts about AI, NLP, and machine learning.
...
\begin{itemize}
    \item HRS $N = 5$ limits statistical power; findings are pilot evidence.
...
We introduced a topic-controlled forensic audit methodology for AI text detectors. A fine-tuned DeBERTa classifier achieves near-perfect discrimination on three of four same-topic human--AI pairs but fails on a meta-recursive pair whose topic is AI detection itself. The synthetic control confirms the methodology. We contribute the HRS protocol, multi-lens framework, and the first documentation of topic-induced detector blindness as tools for future forensic evaluation.
